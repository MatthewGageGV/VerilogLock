...
module LCD_Keypad_Top(clk, reset, row, col, anode, segment, LCD_Out, E, RS, R_W);
...
Keypad              K1  (.clk(clk), .Row(row), .Col(col), .Out0(display_0), .Out1(display_1), .Out2(display_2), .Out3(display_3));

LCD                 L1  (.clk(clk), .reset(reset), .data(LCD_Out), .RS(RS), .R_W(R_W), .E(E));
